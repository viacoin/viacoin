    ...
    const std::string& out_ciphertext_endswith_hex)
{
    ...
    cipher.Initialize(ellswift_theirs, in_initiating, BITCOIN_MAINNET_MESSAGE_START, /*self_decrypt=*/false);
    BOOST_CHECK(cipher);
    ...
    for (unsigned error = 0; error <= 12; ++error) {
        ...
        dec_cipher.Initialize(ellswift_theirs, (error == 1) ^ in_initiating, BITCOIN_MAINNET_MESSAGE_START, /*self_decrypt=*/true);
        BOOST_CHECK(dec_cipher);
...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    // These vectors are specified for Bitcoin mainnet message start, not this fork's main chain.
