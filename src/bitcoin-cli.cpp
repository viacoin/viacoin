...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: viacoin-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to viacoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The viacoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: viacoin-cli [options] <command> [params]\n"
                "or:    viacoin-cli [options] -named <command> [name=value]...\n"
                "or:    viacoin-cli [options] help\n"
                "or:    viacoin-cli [options] help <command>\n"
                "\n";
...
struct AddrinfoRequestHandler : BaseRequestHandler {
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires viacoind server to be running v22.0 and up");
        }
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (const auto res{ToIntegral<uint8_t>(args.at(0))}) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: viacoin-cli -netinfo help", args.at(0)));
            }
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    m_outbound_only_selected = true;
                } else if (n) {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: viacoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: viacoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires viacoind server to be running v0.21.0 and up");
        }
    ...
    const std::string m_help_doc{
        ...
        "> viacoin-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> viacoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> viacoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title viacoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> viacoin-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the viacoind server is running and that you are connecting to the correct RPC port.\n"
