...
//    LogPrintf("vChainMerkleBranch.size() = %d\n", vChainMerkleBranch.size()); // LED TMP
    ...
    CScript::const_iterator pcHead =
            std::search(script.begin(), script.end(), BEGIN(pchMergedMiningHeader), END(pchMergedMiningHeader));
    ...
    if (pcHead != script.end())
    {
        ...
        if (script.end() != std::search(pcHead + 1, script.end(), BEGIN(pchMergedMiningHeader), END(pchMergedMiningHeader)))
