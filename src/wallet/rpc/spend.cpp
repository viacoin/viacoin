    ...
    return RPCHelpMan{
        ...
        HELP_REQUIRING_PASSPHRASE,
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 VIA with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 VIA with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 VIA with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 VIA with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!pwallet->chain().rpcEnableDeprecated("settxfee")) {
        ...
        "\nTo use settxfee restart viacoind with -deprecatedrpc=settxfee.");
    }
        ...
        if (options.type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress") || options.exists("change_address")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid viacoin address");
                }
                ...
                "entire package have the given fee rate, not the resulting transaction.\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The viacoin address to receive the change"},
...
static std::vector<RPCArg> OutputsDoc()
{
    return
    {
        {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
            {
                {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the viacoin address,\n"
...
RPCHelpMan send()
{
    return RPCHelpMan{
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_address", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The viacoin address to receive the change"},
        ...
        RPCExamples{""
        "\nSend 0.1 VIA with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 VIA with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 VIA with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 VIA with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
    ...
    return RPCHelpMan{"sendall",
        ...
        "If your wallet contains many small inputs, either because it received tiny payments or as a result of accumulating change, consider using `send_max` to exclude inputs that are worth less than the fees needed to spend them.\n",
        {
            {"recipients", RPCArg::Type::ARR, RPCArg::Optional::NO, "The sendall destinations. Each address may only appear once.\n"
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "A viacoin address which receives an equal share of the unspecified amount."},
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "changepos", "The position of the added change output, or -1"},
                                ...
                                RPCExamples{
                            "\nCreate a PSBT with automatically picked inputs that sends 0.5 VIA to an address and has a fee rate of 2 via/vB:\n"
