...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Viacoin
    //   new: C:\Users\Username\AppData\Local\Viacoin
    // macOS: ~/Library/Application Support/Viacoin
    // Unix-like: ~/.viacoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Viacoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Viacoin";
#else
    fs::path pathRet;
