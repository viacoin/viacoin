...
The data directory is the default location where the Viacoin node files are stored.
...
---------|--------------------
Linux    | `$HOME/.viacoin/`
macOS    | `$HOME/Library/Application Support/Viacoin/`
Windows  | `%LOCALAPPDATA%\Viacoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `viacoin.conf`, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `viacoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for the Viacoin daemon or GUI. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `viacoind.pid`        | Stores the process ID (PID) of the daemon or GUI while running; created at start and deleted on shutdown; can be specified by `-pid` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [`viacoin.conf`](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
| viacoin.conf                                              | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
...
- README and viacoin.conf files are included in binary releases but not installed in source builds.
