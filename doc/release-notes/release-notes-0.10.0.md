Viacoin Core version 0.10.0 is now available from:
...
backwards-compatible with older versions of Viacoin Core or other software:
...
optimized for the curve Viacoin uses which was created by Bitcoin
...
the curve Viacoin uses and we have reason to believe that
...
Starting from 0.10.0, the Viacoin Core distribution includes a consensus library.

The purpose of this library is to make the verification functionality that is
critical to Viacoin's consensus available to other applications, e.g. to language
bindings such as [python-bitcoinlib](https://pypi.python.org/pypi/python-bitcoinlib) or
alternative node implementations.

This library is called `libbitcoinconsensus.so` (or, `.dll` for Windows).
Its interface is defined in the C header [bitcoinconsensus.h](https://github.com/viacoin/viacoin/blob/master/src/script/bitcoinconsensus.h).
...
While the Viacoin protocol has always supported these types of script,
actually using them on mainnet has been previously inconvenient as
standard Viacoin Core nodes wouldn't relay them to miners, nor would
...
Viacoin Core now supports BIP 22 long polling, so mining software can be
...
The same miner-voting mechanism as in BIP 34 is used: when 75% out of blocks 
have version number 4 or higher, the new consensus rule becomes active for those 
blocks. When 95% out of blocks have version number 4 or higher, it becomes 
