# `viacoin.conf` Configuration File
...
The `settings.json` file contains dynamic settings that are set by the Viacoin GUI and RPCs at runtime, and augment or replace the static settings specified in the `viacoin.conf` file.

Command line options also augment or replace `viacoin.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `viacoin.conf` also show up as warnings in `debug.log` output.
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `viacoin.conf` and it is located in the Viacoin data directory, but both the Viacoin data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.

The `includeconf=<file>` option in the `viacoin.conf` file can be used to include additional configuration files.

### Default configuration file locations

Operating System | Data Directory | Example Path
-- | -- | --
Windows | `%LOCALAPPDATA%\Viacoin\` | `C:\Users\username\AppData\Local\Viacoin\viacoin.conf`
Linux | `$HOME/.viacoin/` | `/home/username/.viacoin/viacoin.conf`
macOS | `$HOME/Library/Application Support/Viacoin/` | `/Users/username/Library/Application Support/Viacoin/viacoin.conf`
...
cp share/examples/bitcoin.conf ~/.viacoin/viacoin.conf
```
